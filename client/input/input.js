...
let keys = {}
const up = 0, released = 1, pressed = 2, down = 3
document.onkeydown = (e) => {
    if (keys[e.key] != down) keys[e.key] = pressed
}
document.onkeyup = (e) => {
    keys[e.key] = released
}
let mousex, mousey, click, attack
let touch = {};
...
let canvas = document.querySelector('canvas')
        var rect = canvas.getBoundingClientRect(), // abs. size of element
            scaleX = 640 / rect.width,    // relationship bitmap vs. element for x
            scaleY = 360 / rect.height;  // relationship bitmap vs. element for y
...
function axisX() {
    return (keys['d'] == down ? 1 : 0) + (keys['a'] == down ? -1 : 0)
}
function axisY() {
    return (keys['s'] == down ? 1 : 0) + (keys['w'] == down ? -1 : 0)
}
...
if(obj.writing){
        for(let i in keys){
            ...
            if(i == 'Enter'  && keys[i] == down && tick % 16 == 0)obj.text += '\n'
            if(i.length == 1 && keys[i] == pressed){
                obj.text += i
            }
        }
    }
    text(ctx, obj.text, font, range.x, range.y, '#45444f')
}